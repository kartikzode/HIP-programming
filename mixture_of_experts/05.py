...
def gate_up_silu_kernel(
    ...
    for g in range(num_experts):
        ...
        while tile_idx >= last_problem_end and tile_idx < last_problem_end + num_tiles:
            ...
            flat_pos = tl.load(dest_row_ptr + row_offset + offs_am, mask=m_mask, other=0)
            token_ids = flat_pos // topk
            # Pointer Arithmetic
...
def down_proj_kernel(
    ...
    for g in range(num_experts):
        ...
        while tile_idx >= last_problem_end and tile_idx < last_problem_end + num_tiles:
            ...
            expert_scores = tl.load(topk_scores_ptr + dest_rows, mask=m_mask, other= 0.0)
...
@triton.jit
def count_token2expert_per_chunck(
    expert_indices_ptr,
    token_counts_ptr,
    num_experts: tl.constexpr,
    numel: tl.constexpr,
    tokens_per_pid: tl.constexpr,
    ):
    pid = tl.program_id(0)
    start_m = pid * tokens_per_pid
    row_offset = (pid + 1) * num_experts

    for i in range(tokens_per_pid):
        if start_m + i < numel:
            idx = tl.load(expert_indices_ptr + start_m + i)
            token_cnt = tl.load(token_counts_ptr + row_offset + idx)
            tl.store(token_counts_ptr + row_offset + idx, token_cnt + 1)

@triton.jit
def count_accumulate_per_expert(
    token_counts_ptr,
    num_experts: tl.constexpr,
):
    pid = tl.program_id(0)
    ...
    for i in range(1, num_experts + 1):
        count = tl.load(token_counts_ptr + i*num_experts + pid)
        current_sum += count
        tl.store(token_counts_ptr + i*num_experts + pid, current_sum)
...
def cumsum_populate(
    ...
    current_sum = 0
    offset_count = num_experts * num_experts
    for i in range(1, num_experts + 1):
        token_count = tl.load(token_counts_ptr + offset_count + i - 1)
        current_sum += token_count
        tl.store(cumsum_ptr + i, current_sum)

@triton.jit
def scatter_tokens_to_slots(
    expert_indices_ptr,
    token_counts_ptr,     # POST accumulate: row `pid` holds "count before chunk pid", per expert
    cumsum_ptr,            # cumsum[e] = starting offset for expert e (0-indexed, length num_experts+1)
    dest_row_ptr,           # OUT: (numel,) int32 -- dest_row[sorted_pos] = original flat (token,k) index
    num_experts: tl.constexpr,
    numel: tl.constexpr,
    tokens_per_pid: tl.constexpr,
):
    ...
    for i in range(tokens_per_pid):
        pos = start_m + i
        if pos < numel:
            expert_id = tl.load(expert_indices_ptr + pos)
            rank = tl.load(token_counts_ptr + row_offset + expert_id)
            base = tl.load(cumsum_ptr + expert_id)
            tl.store(dest_row_ptr + base + rank, pos)
            tl.store(token_counts_ptr + row_offset + expert_id, rank + 1)

def permute_for_experts_custom(
    expert_indices: torch.Tensor,
    num_experts: int,
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    numel = expert_indices.numel()

    token_counts = torch.zeros((num_experts + 1, num_experts),
                               dtype= torch.int32,
                               device=expert_indices.device)
    cumsum = torch.zeros((num_experts + 1, ),
                         dtype=torch.int32,
                         device= expert_indices.device)
    dest_row = torch.empty((numel, ), dtype=torch.int32, device=expert_indices.device)
    ...
    offsets = cumsum[:num_experts].contiguous()
    return counts, offsets, dest_row
...
def moe_forward(input_tensor: torch.Tensor,
    ...
    counts, expert_offsets, dest_row = permute_for_experts_custom(topk_indices, num_experts)
    # dest_row : (batch_size * seq_len * n_experts_per_token, )
    # counts : [num_experts]
    # expert_offsets : [num_experts]

    exp_token_pairs = topk_indices.numel()
