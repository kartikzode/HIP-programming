...
def expert_kernel(
    ...
    # idxs : (batch_size*seq_len*n_experts_per_token, ) -> sort_order_indices
    # token_idxs : (batch_size * seq_len * n_experts_per_token, ) -> tokens sorted according to the experts
    # sorted_exp_ids : (batch_size * seq_len * n_experts_per_token, ) : (1, )
    # sorted_exp_scores : (batch_size * seq_len * n_experts_per_token, ) : (1, 1)
    # num_expert_token_pairs : (batch_size * seq_len * n_experts_per_token, ) : (1, )
    # grid : (cdiv(num_expert_token_pairs, BM) * cdiv(d_expert, BN), )

    pid = tl.program_id(axis=0)
    num_pid_m = tl.cdiv(num_expert_token_pairs, BLOCKSIZE_M)
    ...
    offs_am = tl.where(offs_am < num_expert_token_pairs, offs_am, 0)
    ...
    expert_id = tl.load(sorted_expert_ids + offs_am)
    expert_score = tl.load(sorted_expert_scores + offs_am)
    expert_offset = expert_id * d_hidden * d_expert
    token_ids = tl.load(token_idxs + offs_am)
    
    # Pointer Arithmetic
    a_ptrs = x + (token_ids[:, None] * d_hidden + offs_k[None, :])               # [BM, BK]
    b0_ptrs = W_gate + expert_offset + offs_k[:, None] * d_expert + offs_b0n[None, :]          # [BK, BN]
    b1_ptrs = W_up + expert_offset + offs_k[:, None] * d_expert + offs_b1n[None, :]            # [BK, BN]
    b2_ptrs = W_down + expert_offset + offs_b2n[:, None] * d_hidden + offs_k[None, :]          # [BN, BK]
    ...
    for k in range(0, tl.cdiv(d_hidden,BLOCKSIZE_K)):
        ...
        v = v * expert_score[:, None]         # [BM, BK] * [BM, BK]

        # writing to the out matrix
        tl.atomic_add(
            out + token_ids[:, None] * d_hidden + (k*BLOCKSIZE_K + offs_k[None, :]),
            v,
            mask= (offs_cm[:, None] < num_expert_token_pairs) & (offs_k[None, :] < d_hidden - k * BLOCKSIZE_K),
        )
        b2_ptrs += BLOCKSIZE_K
...
def moe_forward(input_tensor: torch.Tensor,
    ...
    num_expert_token_pairs = num_tokens * n_experts_per_token
    # num_token_expert_pairs : (batch_size * seq_len * n_experts_per_token)
    ...
    idxs = flat_expert_indices.argsort()    # (batch_size * seq_len * n_experts_per_token, )
    sorted_expert_ids = flat_expert_indices[idxs]
    sorted_expert_scores = flat_expert_scores[idxs]
    token_idxs = idxs // n_experts_per_token  # (batch_size * seq_len * n_experts_per_token, )

    grid = lambda META: (num_expert_token_pairs * triton.cdiv(d_expert, META["BLOCKSIZE_N"]), )
