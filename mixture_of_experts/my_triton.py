...
def expert_kernel(
    ...
    scales = tl.load(
        exp_weights + start_m + tl.arange(0, BLOCKSIZE_M),
        mask= tl.arange(0, BLOCKSIZE_M) < idxs_len - start_m
    )
    
    # Pointer Arithmetic
    a_ptrs = x + (token_ids[:, None] * d_hidden + offs_k[None, :])
    b0_ptrs = exp_gate + (offs_k[:, None] * d_expert + offs_b0n[None, :])
    b1_ptrs = exp_up + (offs_k[:, None] * d_expert + offs_b1n[None, :])
    b2_ptrs = exp_down + (offs_b2n[:, None] * d_hidden + offs_k[None, :])
...
class MoE(nn.Module):
    ...
    def moe_infer(self,
        ...
        for expert_id, end_idx in enumerate(tokens_per_expert):
            ...
            grid = lambda META: (triton.cdiv(len(exp_token_idxs), META["BLOCKSIZE_M"]) * triton.cdiv(self.config["d_expert"], META["BLOCKSIZE_N"]), )
            expert_kernel[grid](
